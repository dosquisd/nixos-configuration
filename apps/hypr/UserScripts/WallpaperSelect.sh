...
iDIRi="/etc/nixos/apps/swaync/icons"
...
# Variables
rofi_theme="/etc/nixos/apps/rofi/config-wallpaper.rasi"
...
# Monitor details
scale_factor=$(hyprctl monitors | awk -v mon="$focused_monitor" '
  /^Monitor / { inblock = ($2 == mon) }
  inblock && /scale:/ { print $2; exit }
')
monitor_height=$(hyprctl monitors | awk -v mon="$focused_monitor" '
  /^Monitor / { inblock = ($2 == mon) }
  inblock && /height:/ { print $2; exit }
')
...
modify_startup_config() {
  ...
  local startup_config="/etc/nixos/apps/hypr/UserConfigs/Startup_Apps.conf"
