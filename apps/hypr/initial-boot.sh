...
wallpaper=/etc/nixos/apps/hypr/wallpaper_effects/.wallpaper_current
waybar_style="/etc/nixos/apps/waybar/style/[Extra] Neon Circuit.css"
...
if [ ! -f "/etc/nixos/apps/hypr/.initial_startup_done" ]; then
    sleep 1
	...
	# waybar style
	#if [ -L "/etc/nixos/apps/waybar/config" ]; then
    ##    	ln -sf "$waybar_style" "/etc/nixos/apps/waybar/style.css"
    #   	"$scriptsDir/Refresh.sh" > /dev/null 2>&1 & 
	#fi


    # Create a marker file to indicate that the script has been executed.
    touch "/etc/nixos/apps/hypr/.initial_startup_done"
