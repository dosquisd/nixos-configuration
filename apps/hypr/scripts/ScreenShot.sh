...
iDIR="/etc/nixos/apps/swaync/icons"
iDoR="/etc/nixos/apps/swaync/images"
sDIR="/etc/nixos/apps/hypr/scripts"

if [[ -n "$JQ_BIN" ]]; then
	active_window_class=$(hyprctl -j activewindow | "$JQ_BIN" -r '(.class)')
else
	active_window_class=$(hyprctl activewindow | awk -F'class: ' '/class: /{print $2; exit}' | xargs)
fi
...
shotactive() {
	...
	if [[ -n "$JQ_BIN" ]]; then
		hyprctl -j activewindow | "$JQ_BIN" -r '"\(.at[0]),\(.at[1]) \(.size[0])x\(.size[1])"' | grim -g - "${active_window_path}"
	else
		hyprctl activewindow | awk '
			...
			END { if (x != "" && y != "" && w != "" && h != "") print x "," y " " w "x" h; }
		' | grim -g - "${active_window_path}"
	fi
	sleep 1
    notify_view "active"
}
